...
### Overview
Kroger/Ralph's cart integration that enables users to match recipe ingredients to store products, automatically clip digital coupons, and pre-fill their shopping cart for checkout.

**Status**: Planned feature
**API**: Kroger Developer API (free, OAuth-based)
**Target Tab**: Grocery tab
...
- User has Kroger Boost membership
- Store determined automatically by zip code from user's Kroger account
- No manual store selector needed
- App uses whatever store Kroger's API returns for the account location

---

### UI Flow (3 Screens)

#### 1. Product Matching Screen

**Per-Item Options (3 choices):**
- **Best match**: Closest to recipe ingredient
- **Store brand**: Cheaper alternative
- **Bigger size**: Better price per unit (ideal for meal prep)

**UI Elements:**
- Shows per-unit price for all options (e.g., `$3.66/lb`)
- **"I have this" toggle** per item:
  - Collapses options when toggled
  - Excludes item from cart
  - Button label changes to "Add back" when excluded
  - Collapsed state stays visible (not deleted) so user can restore
- **Not-found items**: Show Claude-suggested substitution instead of dead end

#### 2. Auto Coupon Clipping Screen

**Flow:**
- After product matching, automatically fetch all available digital coupons
- Clip coupons in background
- Show live status per item: `checking → clipped / no coupon`

**API Endpoints:**
- `GET /v1/coupons` - Fetch available coupons
- `POST /v1/coupons/clip` - Clip coupon to account

**Additional Savings:**
- Kroger Plus sale prices applied automatically via linked account

#### 3. Cart Summary + Checkout

**Summary Display:**
- Each item with: original price → discount → final price
- **Savings breakdown**:
  - Regular total
  - Kroger Plus savings
  - Coupon savings
...
**Checkout:**
- Final button opens Ralph's/Kroger website with cart pre-filled
- User completes checkout themselves (never automated)
...
**Registration:**
- Free API access at `developer.kroger.com`
- OAuth for user authentication
- Tokens stored in localStorage (one-time login)

**OAuth Setup:**
- Connects to user's existing Ralph's/Kroger account
- Includes keychain card (same account, no extra setup)
- Redirect URIs:
  - Dev: `http://localhost:5173/callback`
  - Prod: `https://yourdomain.com/callback`

**API Endpoints:**

| Method | Endpoint | Purpose |
|--------|----------|---------|
| GET | `/v1/identity/profile` | Get user profile + zip code |
| GET | `/v1/locations?filter.zipCode=...&filter.radiusInMiles=10` | Get closest store |
| GET | `/v1/products?filter.term=...&filter.locationId=...` | Search products |
| PUT | `/v1/cart/add` | Add items to cart |
| GET | `/v1/coupons` | Fetch available coupons |
| POST | `/v1/coupons/clip` | Clip coupons to account |

**API Response Data:**
- Product name, price, unit price, size, UPC
- Kroger Plus discount price
- **Does NOT return**: Near-expiration markdowns (physical in-store only)
...
### Physical Keychain Card

**How It Works:**
- Already linked to Kroger account via OAuth login
- For **delivery**: Irrelevant, Boost handles everything via account
- For **in-store**:
  - Scan keychain card or app barcode at register as usual
  - Digital coupons clipped via app apply automatically at checkout
...
**API Layer:**
- `src/kroger.js` - All Kroger API calls (auth, search, cart, coupons)

**UI Components:**
- `src/components/KrogerAuth.jsx` - OAuth connect button + token storage
- `src/components/ProductMatcher.jsx` - Per-item matching with 3 options + toggle
- `src/components/CouponClipper.jsx` - Auto clip animation + status
- `src/components/CartSummary.jsx` - Final cart + savings breakdown + checkout button
...
**Authentication Flow:**
1. User clicks "Connect Kroger" button
2. OAuth popup redirects to Kroger login
...
1. Parse grocery list from meal plan
2. For each ingredient, query `/v1/products` API
3. Present 3 options: best match, store brand, bigger size
4. Track user selections and "I have this" exclusions
5. Build cart payload from selections

**Coupon Clipping Flow:**
1. Fetch all available coupons for selected products
2. Batch clip all matching coupons
3. Display real-time status per item
4. Calculate final savings in cart summary

**Cart Pre-fill:**
1. Use `/v1/cart/add` endpoint with selected product UPCs
2. Generate deep link to Kroger/Ralph's cart page
3. User clicks "Checkout" to open in new tab
4. User completes purchase on Kroger website
